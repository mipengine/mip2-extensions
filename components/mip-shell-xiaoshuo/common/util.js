...
export const getJsonld = (currentWindow) => {
  ...
  // 每个页面不同，如上一页链接，当前章节名
  let jsonld
  if (currentWindow.document && currentWindow.document.head) {
    jsonld = currentWindow.document.head.querySelector("script[type='application/ld+json']")
  } else {
    jsonld = currentWindow.querySelector("script[type='application/ld+json']")
  }
